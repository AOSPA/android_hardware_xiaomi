...
class DolbyAudioEffect(priority: Int, audioSession: Int) :
    ...
    fun setDapParameter(param: DsParam, values: IntArray) {
    ...
    fun setDapParameter(param: DsParam, enable: Boolean) {
        setDapParameter(param, intArrayOf(if (enable) 1 else 0))
    }

    fun setDapParameter(param: DsParam, value: Int) {
        setDapParameter(param, intArrayOf(value))
    }

    fun getDapParameter(param: DsParam): IntArray {
        dlog("getDapParameter: profile=$profile param=$param")
        val length = param.length
        val buf = ByteArray((length + 2) * 4)
        val param = (param.id shl 16) + (profile shl 8) + EFFECT_PARAM_GET_PROFILE_PARAMETER
    ...
    fun getDapParameterInt(param: DsParam): Int {
